...
class AIMechanicCore:
    def __init__(self, embeddings_file: str = "chunks_with_embeddings.json"):
    ...
    def build_context_prompt(self, query: str, similar_chunks: List[Dict]) -> str:
        """Build the context prompt for GPT-4o"""
        context_sections = []
...
- If the manual does not provide an exact answer, say: "The manual does not specify. Please consult your team or manufacturer."
    ...
    def query_gpt4o(self, prompt: str) -> str:
        ...
        try:
            response = self.openai_client.chat.completions.create(
                model="gpt-4o",
                messages=[
                    {"role": "system", "content": "You are an expert AI Mechanic specializing in Radical SR3 race cars. Provide accurate, technical answers based only on the provided manual content."},
...
def main():
    """Test the AI Mechanic system"""
    try:
        ai_mechanic = AIMechanicCore()
        
        # Test questions
        test_questions = [
            "How do I check the oil level?",
            "What is the tire pressure for Hankook tires?",
            "How do I adjust the seat and pedals?",
            "What should I check before starting the engine?",
            "How do I set up the car for wet weather?"
        ]
        
        print("=== AI Mechanic Test ===\n")
